...
impl TryFrom<&'static str> for Backend {
    ...
    fn try_from(value: &'static str) -> Result<Self> {
        match value {
            ...
            _ => Err(anyhow!("unknown container backend")),
        }
    }
}
...
impl ContainerImage {
    ...
    pub const fn new(backend: Backend, id: String, remove_on_drop: bool) -> Self {
        Self {
            backend,
            id,
            remove_on_drop,
    ...
    pub fn build(backend: Backend, containerfile: &Path) -> Result<Self> {
...
impl Step<LinuxVMBuildContext> for CopyFilesystem {
    fn run(&mut self, ctx: &mut LinuxVMBuildContext) -> Result<()> {
        ...
        let rootfs = ctx.0.get::<RootFS>("rootfs").ok_or(Error::invalid_context(
            "build root filesystem",
...
impl BuildContainerImage {
    pub fn new(backend: Backend, containerfile: PathBuf) -> Self {
        Self {
            backend,
            containerfile,
        }
...
impl Step<LinuxVMBuildContext> for GetContainerRuntime {
    fn run(&mut self, ctx: &mut LinuxVMBuildContext) -> Result<()> {
        if let Some(image) = ctx.0.get::<ContainerImage>("container-image") {
            ...
            ctx.0.set("container-rt-config", Box::new(rt_config));
        }

...
impl Step<LinuxVMBuildContext> for InstallMia {
    fn run(&mut self, ctx: &mut LinuxVMBuildContext) -> Result<()> {
        ...
        let container_rt_config = ctx.0.get::<RuntimeConfig>("container-rt-config").ok_or(
            LinuxVMBuilderError::invalid_context("install MIA", "container runtime config"),
        )?;
        let kernel_modules = ctx
            .0
